...
image_path = Path("Image\logo_co2_calculator.png")
...
# Section 3. Overview des modèles possibles
   ...
   # Section Overview des modèles possibles
   st.markdown("""
   ...
   # Etapes précédentes (features engineering + pre-processing)
   ...
   # Chargement des modèles enregistrés avec joblib
   et_regressor = joblib.load("model_et_regressor")
   xgb_regressor = joblib.load("model_xgb_regressor")
   rf_regressor = joblib.load("model_rf_regressor")


   # Fonction pour calculer les métriques d'un modèle donné
   def calculate_metrics(model, X_train, X_test, y_train, y_test):
      ...
      # Résultats au format dictionnaire pour être ajoutés au DataFrame final
      metrics = {
         'R2 train': r2_train,
         'R2 test': r2_test,
         'MAE train': mae_train,
...
if page == pages[4] : 
   ...
   # Afficher le GIF dans Streamlit
   st.image(gif_path_str)


   # DEMONSTRATION
   # Charger les objets joblib avec les nouveaux noms
   X_train = joblib.load('X_train')
   X_train_concat = joblib.load('X_train_concat')  # Charger les données d'entraînement
   et_regressor = joblib.load("et_regressor")  # Charger le modèle Extra Trees
